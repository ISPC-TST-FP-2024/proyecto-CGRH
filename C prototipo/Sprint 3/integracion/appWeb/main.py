...
# --------------------------------------------------------------#
# -- Bloques que cargan datos desde la BD ----------------------#

@app.route('/api/datos', methods=['POST'])
def recibir_datos():
    try:
        # Obtener los datos enviados desde el ESP32 en formato JSON
        ...
        luz = data.get('luz')
        hum_cap = data.get('hum_cap')
        hum_res = data.get('hum_res')
        ...
        query = """
            INSERT INTO sensor_datos (dispositivo_id, temperatura, humedad, luz, 
                                      humedad_capacitiva, humedad_resistiva, 
                                      nivel_agua, distancia, iluminacion, bomba, fecha)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW())
        """
        cursor.execute(query, (nodo_id, temperatura, humedad, luz, hum_cap, 
                               hum_res, nivel_agua, distancia, iluminacion, bomba))
        conexion.commit()
...
# --------------------------------------------------------------#
# -- Bloques que llaman info desde la BD -----------------------#
